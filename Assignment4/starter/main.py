...
class MRV(VarSelector):
    ...
    def select_variable(self, grid):
        ...
        for i in range(grid.get_width()):
            for j in range(grid.get_width()):
                domainLen = len(grid.get_cells()[i][j])
                if domainLen  < minDomain and domainLen > 1:
                    minDomain = domainLen
                    minRow = i
                    minCol = j
...
class AC3:
    ...
    def consistency(self, grid, Q):
        ...
        while Q:
            ...
            variables_assigned1, failures1 = self.remove_domain_row(grid,var[0],var[1])
            variables_assigned2, failures2= self.remove_domain_column(grid,var[0],var[1])
            variables_assigned3, failures3 = self.remove_domain_unit(grid,var[0],var[1])
            
            if (failures1 == True) or (failures2 == True) or (failures3 == True):
                return True
            
            
            variablesAssigned = variables_assigned1 + variables_assigned2 
            variablesAssigned2 = variablesAssigned + variables_assigned3
            
            for var in variablesAssigned2:
...
class Backtracking:
    ...
    def search(self, grid, var_selector):
        ...
        """
        ac3 = AC3()
        # preResult = ac3.pre_process_consistency(grid)
        ...
        domains = grid.get_cells()[var[0]][var[1]]
        for domain in domains:
            
            if grid.is_value_consistent(domain, var[0], var[1])== False:
                continue
            copy_g = grid.copy()
            copy_g.get_cells()[var[0]][var[1]] = domain
            ri = ac3.consistency(copy_g, [(var[0], var[1])])
            if ri == True:
                continue
...
for p in problems:
    ...
    start= default_timer()
    faSearch = backTrack.search(g, firstAvailable)
    running_time_first_available.append(default_timer() - start)
    print('Is the current grid a solution (FA)? ', faSearch.is_solved())
...
for p in problems:
    ...
    start= default_timer()
    mrvSearch = backTrack.search(g, mrv)
    running_time_mrv.append(default_timer() - start)
    
    print('Is the current grid a solution (MRV)? ', mrvSearch.is_solved())
